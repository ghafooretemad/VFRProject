...
class UsersList(QtWidgets.QWidget):
    ...
    def setupUi(self, Form):
        ...
        font.setWeight(75)
        item.setFont(font)
        self.usersList.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        item.setFont(font)
        self.usersList.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        item.setFont(font)
        self.usersList.setHorizontalHeaderItem(5, item)
        item = QtWidgets.QTableWidgetItem()
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        item.setFont(font)
        self.usersList.setHorizontalHeaderItem(6, item)
    ...
    def retranslateUi(self, Form):
        ...
        item = self.usersList.horizontalHeaderItem(3)
        item.setText(_translate("Form", "User Name"))
        item = self.usersList.horizontalHeaderItem(4)
        item.setText(_translate("Form", "Edit"))
        item = self.usersList.horizontalHeaderItem(5)
        item.setText(_translate("Form", "Delete"))
        item = self.usersList.horizontalHeaderItem(6)
        item.setText(_translate("Form", "Active"))
    def ShowUsers(self):
        query = self.GetData()
        self.usersList.setRowCount(query.size())
        row = 0
        ...
        while query.next():
            self.usersList.setItem(row, 0, QtWidgets.QTableWidgetItem(str(query.value("ID"))))
            self.usersList.setItem(row, 1, QtWidgets.QTableWidgetItem(query.value("firstName")))
            self.usersList.setItem(row, 2, QtWidgets.QTableWidgetItem(query.value("lastName")))
            self.usersList.setItem(row, 3, QtWidgets.QTableWidgetItem(query.value("email")))
            self.editButton = QtWidgets.QPushButton("edit")
            self.editButton.setIcon(editIcon)
            self.editButton.clicked.connect(self.editData)
            self.usersList.setCellWidget(row, 4, self.editButton)
            self.deleteButton = QtWidgets.QPushButton("delete")
            self.deleteButton.setIcon(deleteIcon)
            self.deleteButton.clicked.connect(self.deleteData)
            self.usersList.setCellWidget(row, 5, self.deleteButton)
            if query.value("active") == 0:
                ...
                self.usersList.setCellWidget(row, 6, self.activeButton)
                ...
                self.usersList.setCellWidget(row, 6, self.deActiveButton)
            row +=1       
    def GetID(self):
        button = self.sender()
        index = self.usersList.indexAt(button.pos())
        if index.isValid():
            try:
                row = int(index.row())
                ID= self.usersList.item(row,0)
                return ID.text()
            except:
